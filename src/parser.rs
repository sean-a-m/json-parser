...
fn parse_terminal(token: &Token) -> Box<Json> {
    ...
    let json_value = JsonTerminal{val: json_string, json_type: token_type};
    Box::new(json_value)
} 
...
pub fn parse_value<'a, I>(token: &Token, mut tokens: &mut I) -> Box<Json> 
    where I: Iterator<Item=&'a Token> {
    match token.enum_type {
        ...
        _ => {println!("none!"); parse_object(tokens)},
    }
